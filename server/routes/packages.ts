...
router.post("/buy", authMiddleware, async (req: any, res: Response) => {
  try {
    ...
    const referralColumnResult = await db.exec(
    ...
    const packageExistsResult = await db.exec(
    ...
    if (upline.level1) {
      ...
      const level1Result = await db.exec(
        "SELECT usdt_balance FROM wallets WHERE user_id = $1",
        [upline.level1]
      );
    ...
    if (upline.level2) {
      ...
      const level2Result = await db.exec(
        "SELECT usdt_balance FROM wallets WHERE user_id = $1",
        [upline.level2]
      );
    ...
    if (upline.level3) {
      ...
      const level3Result = await db.exec(
        "SELECT usdt_balance FROM wallets WHERE user_id = $1",
        [upline.level3]
      );
