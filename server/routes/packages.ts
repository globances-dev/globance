...
// ======================
// BUY PACKAGE (FULL FIXED VERSION)
// ======================
router.post("/buy", authMiddleware, async (req: any, res: Response) => {
  try {
    console.log("[BUY_PACKAGE] Starting purchase");

    // Validate request
    const { package_id, amount } = z.object({
      package_id: z.string(),
      amount: z.number().optional(),
    }).parse(req.body);

    const configId = package_id.trim().toLowerCase();
    console.log("[BUY_PACKAGE] Selected:", configId);
    ...
    const insertResult = await pool.query(
      `INSERT INTO purchases (user_id, package_id, amount, status, created_at, end_time)
       VALUES ($1, $2, $3, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP + INTERVAL '${pkgConfig.duration_days} days')
       RETURNING id`,
...
// GET USER PURCHASES
// ======================
router.get("/user/purchases", authMiddleware, async (req: any, res: Response) => {
  try {
    const pool = getPostgresPool();
    const result = await pool.query(
