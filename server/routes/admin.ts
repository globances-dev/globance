...
router.get("/analytics", adminMiddleware, async (req: any, res: Response) => {
  try {
    ...
    // Total users
    const usersResult = await db.exec("SELECT COUNT(*) as count FROM users");
    const totalUsers = parseInt(usersResult.rows[0].count) || 0;

    // Active miners (active packages count)
    const activeMinersResult = await db.exec(
    ...
    const payouts24hResult = await db.exec(
    ...
    const payouts7dResult = await db.exec(
    ...
    const referralResult = await db.exec(
...
router.post(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      try {
        const userResult = await db.exec("SELECT email FROM users WHERE id = $1", [withdrawal.user_id]);
...
router.get("/referrals", adminMiddleware, async (req: any, res: Response) => {
  try {
    ...
    // Top referrers
    const topReferrersResult = await db.exec(`
    ...
    const bonusResult = await db.exec(
      "SELECT COALESCE(SUM(amount), 0) as total FROM referral_bonus_transactions"
    );
...
router.get("/settings", adminMiddleware, async (req: any, res: Response) => {
  try {
    const db = getSupabaseQueryClient();
    const result = await db.exec("SELECT * FROM settings ORDER BY category, key");
...
router.get("/users/:user_id", adminMiddleware, async (req: any, res: Response) => {
  try {
    ...
    const purchasesResult = await db.exec(`
    ...
    // Get user's deposits
    const depositsResult = await db.exec(`
      SELECT * FROM deposits WHERE user_id = $1 ORDER BY created_at DESC
    `, [req.params.user_id]);

    // Get user's withdrawals
    const withdrawalsResult = await db.exec(`
